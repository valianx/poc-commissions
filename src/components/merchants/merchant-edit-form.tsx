...
// Edit schema - only editable fields
...
type MerchantEditFormData = z.infer<typeof merchantEditSchema>;
...
export function MerchantEditForm({ merchant, onCancel }: MerchantEditFormProps) {
  const router = useRouter();
  const { updateMerchant } = useMerchantsStore();
  ...
  const {
    ...
    defaultValues: {
      ...
      balanceEvaluationEnabled: merchant.balanceEvaluationEnabled,
      depositCallbackUrl: merchant.depositCallbackUrl,
      withdrawalCallbackUrl: merchant.withdrawalCallbackUrl,
      callbackApiKeyRef: merchant.callbackApiKeyRef,
      callbackSecretKeyRef: merchant.callbackSecretKeyRef,
    },
  });
  ...
  const onSubmit = async (data: MerchantEditFormData) => {
    try {
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <CardContent className="space-y-4">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <div className="space-y-2">
              <Label>Nombre del Merchant</Label>
              <Input
                value={merchant.name}
                disabled
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          {/* Editable: Withdrawal Callback URL */}
          <div className="space-y-2">
